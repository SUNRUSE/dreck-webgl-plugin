describe(`webGlRenderIndexed`, () => {
  ...
  describe(`when the vertex buffer is from another context`, () => {
    ...
    let indexBufferThrowIfDisposed: jasmine.Spy;
    let indexBufferThrowIfFromAnotherContext: jasmine.Spy;
    let indexBufferGetInstance: jasmine.Spy;
    ...
    beforeAll(() => {
      ...
      indexBufferThrowIfDisposed = jasmine.createSpy(
        `indexBufferThrowIfDisposed`
      );
      indexBufferThrowIfFromAnotherContext = jasmine.createSpy(
        `indexBufferThrowIfFromAnotherContext`
      );
      indexBufferGetInstance = jasmine.createSpy(`indexBufferGetInstance`);
      const indexBuffer = {
        context,
        throwIfDisposed: indexBufferThrowIfDisposed,
        throwIfFromAnotherContext: indexBufferThrowIfFromAnotherContext,
        getInstance: indexBufferGetInstance,
      };
    ...
    it(`does not check whether the index buffer has been disposed of`, () => {
      expect(indexBufferThrowIfDisposed).not.toHaveBeenCalled();
    });

    it(`does not check whether the index buffer is from another context`, () => {
      expect(indexBufferThrowIfFromAnotherContext).not.toHaveBeenCalled();
    });

    it(`does not get an instance of the index buffer`, () => {
      expect(indexBufferGetInstance).not.toHaveBeenCalled();
    });
  ...
  describe(`when the index buffer is from another context`, () => {
    let useProgram: jasmine.Spy;
    let bindBuffer: jasmine.Spy;
    let vertexAttribPointer: jasmine.Spy;
    let enableVertexAttribArray: jasmine.Spy;
    ...
    it(`throws the expected error`, () => {
      expect(error).toEqual(
        new Error(`The index buffer must be of the given context.`)
      );
    });
  });
  ...
  describe(`when it is not possible to get an instance of the vertex buffer`, () => {
    ...
    beforeAll(() => {
      ...
      indexBufferGetInstance = jasmine
        .createSpy(`indexBufferGetInstance`)
        .and.returnValue({
          buffer: { test: `indexBuffer` },
          data: { count: 224, type: WebGlConstants.UnsignedShort },
        });
      const indexBuffer = {
        context,
        throwIfDisposed: indexBufferThrowIfDisposed,
    ...
    it(`may get one instance of the index buffer`, () => {
      expect(indexBufferGetInstance.calls.count()).toBeLessThanOrEqual(1);
    });
  ...
  describe(`when it is not possible to get an instance of the index buffer`, () => {
    let useProgram: jasmine.Spy;
    let bindBuffer: jasmine.Spy;
    let vertexAttribPointer: jasmine.Spy;
    let enableVertexAttribArray: jasmine.Spy;
  ...
  describe(`when successful`, () => {
    ...
    beforeAll(() => {
      ...
      enableVertexAttribArray = jasmine.createSpy(`enableVertexAttribArray`);
      disableVertexAttribArray = jasmine.createSpy(`disableVertexAttribArray`);
      drawElements = jasmine.createSpy(`drawElements`).and.callFake(() => {
        textureCallOrderValue.push({ type: `drawElements` });
      });
      uniform1f = jasmine.createSpy(`uniform1f`);
      uniform1fv = jasmine.createSpy(`uniform1fv`);
      uniform2f = jasmine.createSpy(`uniform2f`);
    ...
    it(`binds two buffers`, () => {
      expect(bindBuffer).toHaveBeenCalledTimes(2);
    });

    it(`binds the index buffer`, () => {
    ...
    it(`draws the expected elements`, () => {
    ...
    it(`gets one instance of the index buffer`, () => {
      expect(indexBufferGetInstance).toHaveBeenCalledTimes(1);
    });
    ...
    it(`draws arrays after setting vertex attribute pointers`, () => {
      expect(vertexAttribPointer).toHaveBeenCalledBefore(drawElements);
    });
    ...
    it(`draws arrays after setting 1-dimensional float uniforms`, () => {
      expect(uniform1f).toHaveBeenCalledBefore(drawElements);
    });

    it(`draws arrays after setting 1-dimensional float uniform arrays`, () => {
      expect(uniform1fv).toHaveBeenCalledBefore(drawElements);
    });

    it(`draws arrays after setting 2-dimensional float uniforms`, () => {
      expect(uniform2f).toHaveBeenCalledBefore(drawElements);
    });

    it(`draws arrays after setting 2-dimensional float uniform arrays`, () => {
      expect(uniform2fv).toHaveBeenCalledBefore(drawElements);
    });

    it(`draws arrays after setting 3-dimensional float uniforms`, () => {
      expect(uniform3f).toHaveBeenCalledBefore(drawElements);
    });

    it(`draws arrays after setting 3-dimensional float uniform arrays`, () => {
      expect(uniform3fv).toHaveBeenCalledBefore(drawElements);
    });

    it(`draws arrays after setting 4-dimensional float uniforms`, () => {
      expect(uniform4f).toHaveBeenCalledBefore(drawElements);
    });

    it(`draws arrays after setting 4-dimensional float uniform arrays`, () => {
      expect(uniform4fv).toHaveBeenCalledBefore(drawElements);
    });

    it(`draws arrays after setting 2-dimensional matrix uniforms`, () => {
      expect(uniformMatrix2fv).toHaveBeenCalledBefore(drawElements);
    });

    it(`draws arrays after setting 3-dimensional matrix uniforms`, () => {
      expect(uniformMatrix3fv).toHaveBeenCalledBefore(drawElements);
    });

    it(`draws arrays after setting 4-dimensional matrix uniforms`, () => {
      expect(uniformMatrix4fv).toHaveBeenCalledBefore(drawElements);
    });

    it(`draws arrays after setting 1-dimensional integer uniforms`, () => {
      expect(uniform1i).toHaveBeenCalledBefore(drawElements);
    });

    it(`draws arrays after setting 1-dimensional integer uniforms`, () => {
      expect(uniform1iv).toHaveBeenCalledBefore(drawElements);
    });

    it(`disables vertex attribute arrays after drawing arrays`, () => {
      expect(drawElements).toHaveBeenCalledBefore(disableVertexAttribArray);
    });
  });
});
